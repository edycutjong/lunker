 ...
 *
 * The page is static: `scripts/build-site.mjs` renders it into `site/`, which
 * GitHub Pages publishes at https://lunker.edycu.dev (with the privacy policy,
 * images and the pitch deck). Only the live ledger stays on the Worker, so
 * /verify links and the stats fetch use the absolute API_ORIGIN.
 */
...
export const ORIGIN = 'https://lunker.edycu.dev';

/** The Worker: the live ledger (/verify) and the game API. */
...
<script>
  ...
  fetch('${API_ORIGIN}/verify?format=json')
